...
document.addEventListener('DOMContentLoaded', function() {
    ...
    loadNotes();
    loadApps();
    // Eventos do formulário de app
    const showBtn = document.getElementById('showAddAppFormBtn');
    ...
    if (showBtn && form && cancelBtn && addBtn) {
        showBtn.onclick = () => { form.style.display = 'block'; };
        cancelBtn.onclick = () => { form.style.display = 'none'; };
        addBtn.onclick = async () => {
            const name = document.getElementById('appName').value.trim();
            const description = document.getElementById('appDescription').value.trim();
            const icon = document.getElementById('appIcon').value.trim() || 'fas fa-external-link-alt';
            ...
            const res = await fetch('/api/apps', {
                ...
                body: JSON.stringify({name, description, icon, url})
            });
            if (res.ok) {
...
async function loginQAcademico() {
    ...
    showNotification('Iniciando login automático...', 'info');
    ...
    try {
        ...
        if (result.success) {
            showNotification(result.message || 'Login realizado com sucesso!', 'success');
...
async function loadApps() {
    ...
    if (apps.length === 0) {
        container.innerHTML = '<p style="text-align:center;color:#666;">Nenhuma aplicação cadastrada.</p>';
        return;
    }
    apps.forEach(app => {
        ...
        div.innerHTML = `
            ...
            <div class="app-info">
                ...
                <div class="app-actions">
                    <a href="${app.url}" target="_blank" class="app-btn secondary">
                        <i class="fas fa-external-link-alt"></i> Abrir
                    </a>
                    <button class="app-btn danger" onclick="deleteApp(${app.id})">
...
window.deleteApp = async function(id) {
    if (!confirm('Tem certeza que deseja remover esta aplicação?')) return;
